...
export default function ActividadesPage() {
  ...
  const fetchUsuarios = useCallback(async () => {
    const { data, error } = await supabase.from('usuarios').select('id, nombre, apellidos').eq('activo', true);
    if (!error && data) setUsuarios(data as Usuario[]);
  }, []);

  // Cargar Áreas y contar cuántos proyectos tienen asignados en proyecto_areas
  const fetchAreas = useCallback(async () => {
    setLoading(true);
    setErrorMsg('');

    try {
      // 1. Obtener Áreas con el nombre de su responsable
      ...
      // 2. Obtener la relación de proyectos asignados por área
      const { data: relData } = await supabase.from('proyecto_areas').select('area_id');

      // Calcular número de proyectos vinculados por área
      const counts: Record<string, number> = {};
      if (relData) {
        relData.forEach((rel) => {
          counts[rel.area_id] = (counts[rel.area_id] || 0) + 1;
        });
      }

      const formattedAreas: Area[] = (areasData || []).map((a) => ({
        ...a,
        total_proyectos: counts[a.id] || 0
      }));

      setAreas(formattedAreas);
    } catch (err) {
      const error = err as Error;
      setErrorMsg('Error al consultar las áreas: ' + (error.message || 'Verifica la conexión a la base de datos'));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function handleDeleteArea(area: Area) {
    if (area.total_proyectos && area.total_proyectos > 0) {
      alert(`No se puede eliminar la estación "${area.nombre}" porque tiene ${area.total_proyectos} proyecto(s) asignado(s). En su lugar, puedes desactivarla.`);
      return;
    }
  ...
  return (
    <div className="p-8 space-y-6 bg-[#0B0F17] min-h-screen text-slate-100">
      ...
      <form onSubmit={handleSubmit} className="bg-[#121824] border border-slate-800 p-6 rounded-2xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <input
            type="text"
            placeholder="Nombre de la estación (Ej: Diseño, Ensamble)"
            value={nombre}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {areas.map((a) => (
            <div key={a.id} className={`bg-[#121824] border rounded-2xl p-6 space-y-4 transition-all ${a.activo ? 'border-slate-800' : 'border-slate-800/40 opacity-60'}`}>
              ...
              <div className="bg-[#0B0F17] border border-slate-800/60 p-3 rounded-xl space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between text-slate-400 border-t border-slate-800/60 pt-2">
                  ...
                  <span className="font-bold text-red-400">{a.total_proyectos || 0}</span>
                </div>
              </div>
              ...
              <div className="flex justify-end items-center gap-2 pt-2 border-t border-slate-800/60">
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors flex items-center gap-1 text-xs"
                >
                ...
                <button
                  ...
                  className="p-2 text-red-400 hover:text-red-300 hover:bg-red-950/50 rounded-lg transition-colors flex items-center gap-1 text-xs"
                >
